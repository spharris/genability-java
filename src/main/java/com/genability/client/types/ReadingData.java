...
/**
 * This class represents a reading of a measurement. It has a start time and an end time, plus a
 * value and a field that denotes what the value is measuring.
 * 
 */
...
public class ReadingData extends PropertyData implements Serializable {
  ...
  /**
   * Override the default behavior for keyName so that if this has a quantity unit of kWh its set to
   * consumption , and if its kW its set to demand, but only when its not already set (so it can be
  ...
  public String getKeyName() {
    ...
    if (keyName == null) {
      if (ReadingData.KWH_UNIT.equals(this.quantityUnit))
        keyName = "consumption";
      else if (ReadingData.KW_UNIT.equals(this.quantityUnit)) keyName = "demand";
    }
  ...
  public String toString() {
    return "ReadingData [startTime=" + new DateTime(startTime) + ", endTime="
        + new DateTime(endTime) + ", quantityUnit=" + quantityUnit + ", quantityValue="
        + quantityValue + "]";
  }
}
