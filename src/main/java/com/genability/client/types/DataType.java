...
public enum DataType {
  ...
  STRING(1), CHOICE(2), BOOLEAN(4), DATE(8), DECIMAL(16), INTEGER(32), FORMULA(64), LOOKUP(
      128), DEMAND(256);

  private int id;

  /**
   * Constructor, which takes an int value so that this enum's values can be flags (in other words
  ...
  /**
   * Converts the int value passed in into a valid Enum value, or returns null if the int value
   * passed in doesn't yield a match.
   * 
  ...
  /**
   * This method returns a map of messages suitable for display purposes. The maps key is the
   * enumeration value, and the maps value is the corresponding display string.
   * 
   * @return EnumMap of messages
   */
  public static EnumMap<DataType, String> getMessages() {

    EnumMap<DataType, String> messages = new EnumMap<DataType, String>(DataType.class);
